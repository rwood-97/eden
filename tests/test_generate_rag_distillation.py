...
def test_cross_doc_prompt_includes_record_titles():
    ...
    prompt = _cross_doc_prompt(records, source_type="advice", n_questions=3)
...
def test_cross_doc_prompt_requests_correct_n_questions():
    ...
    prompt = _cross_doc_prompt(records, source_type="advice", n_questions=5)
...
def test_cross_doc_prompt_mentions_multiple_records():
    ...
    prompt = _cross_doc_prompt(records, source_type="advice", n_questions=2)

    # Should mention synthesising across records
...
def test_load_template_fills_placeholders():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        tmpl.write_text(
            "Type: {source_type}. Count: {pairs_per_record}. Ideas: {question_ideas}"
        )
